  ...
  interface DonorInfoSectionProps {
    ...
    onDonorResolved: (id: number, name: string) => void
  }
  ...
  export default function DonorInfoSection({
    ...
    const handlePanBlur = async () => {
      ...
      if (!panRegex.test(pan)) {
        setError(
          "PAN format is invalid. Please enter PAN in correct format."
        );
      ...
      try {
        ...
        setDonorId(donor.id);
        onDonorResolved(donor.id, donor.name);
    ...
    const handleCreateDonor = async () => {
      ...
      try {
        ...
        onDonorResolved(newDonorId, name);
        setPanNotFound(false);
        setError("");
        // setTimeout(() => {
